...
class OllamaRunpod(LM):
    ...
    def basic_request(self, prompt: str, **kwargs):
        ...
        # print("BASIC REQUEST", prompt, "PROMPT END!!!")
        ...
        if self.model_type == "chat":
            request_dict["input"]["method_name"] = "chat"
            settings_dict["messages"] = [{"role": "user", "content": prompt}]
        else:
            request_dict["input"]["method_name"] = "generate"
            settings_dict["prompt"] = prompt

        request_dict["input"]["input"] = settings_dict
        urlstr = self.base_url
